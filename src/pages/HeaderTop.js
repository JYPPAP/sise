...
const HeaderTop = () => {
  ...
  const serverInfo = params.serverName
    ? params.gameName + "," + params.serverName
    : "";
  ...
  useEffect(() => {
    ...
    const toggleStar =
      Boolean(Number(favorite.indexOf(serverInfo)) >= 0) || false;
    setStar(toggleStar);
  }, [serverInfo]);
  ...
  // console.log("현재 서버 정보");
  // console.log(serverInfo);
  // console.log("-1이면 노란 별이 false가 떠야 함");
  // console.log(favorite.indexOf(serverInfo));
  // console.log("State 측정");
  // console.log(Boolean(Number(favorite.indexOf(serverInfo)) >= 0));
  // console.log("노란 별");
  // console.log(star);
  // console.log("#####################");
  // console.log("");
  ...
  if (location.pathname === "/") {
    ...
    if (Object.keys(params) !== undefined) {
      title =
        Object.keys(params)[1] === "serverName"
...
function toggleFavorite({ star, setStar, favorite, serverInfo }) {
  if (star) {
    favorite = favorite.replace(
      favorite.indexOf(serverInfo) === 0 ? serverInfo + "|" : "|" + serverInfo,
      ""
    );
  } else {
